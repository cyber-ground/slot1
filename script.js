...
// function initFetch() { //* incase of delay
//   failureId.forEach(async (id, index) => {
//     const key = `${process_env}${uuid}`;
//     const baseURL = `https://api.giphy.com/v1/content/${id}?`; //* byID
...
const fetchImage = document.querySelector('.frame');
  async function loadFailureData(duration) {
    ...
    const data = await response.json(); 
    fetchImage.src = data.data.images.downsized_medium.url;
    fetchImage.style.setProperty('--left', 50 + '%');
    fetchImage.style.setProperty('--bottom', 0 + 'px');
    giphy.style.setProperty('--height', 60 + '%');
...
const option = `&q=search=${search}&limit=${limit}&s=${TL_Word}&tag=${tag}&rating=g`;
const GIPHY_API = `${baseURL}api_key=${key}${option}`;
const giphyLength = 1;
